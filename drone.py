    ...
    while True:
        ...
        try:
            ...
            for code in decoded:
                ...
                polygon = np.array([list(code.polygon[0]), list(code.polygon[1]), list(code.polygon[2]), list(code.polygon[3])], np.int32).reshape((-1, 1, 2))
                center = (int((code.polygon[0].x+code.polygon[1].x+code.polygon[2].x+code.polygon[3].x)/4)-img.shape[1]/2,int((code.polygon[0].y+code.polygon[1].y+code.polygon[2].y+code.polygon[3].y)/4)-img.shape[0]/2)
                if(hover):
                    currentCodeCenter = center
                    #tl_flight.rc(hoverPIDX(center[0]), hoverPIDY(center[1]))
                    print(f"RC X:{hoverPIDX(center[0])}, Y{hoverPIDY(center[1])}")
